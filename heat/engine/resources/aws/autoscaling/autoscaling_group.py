...
from heat.engine.resources.openstack.heat import \
    autoscaling_group as native_asg
...
class AutoScalingGroup(native_asg.AutoScalingResourceGroup):
    ...
    def adjust(self, adjustment,
        ...
        return super(AutoScalingGroup, self).adjust(
            adjustment, adjustment_type, min_adjustment_step, cooldown)

    def _get_resource_definition(self):
        return instgrp.InstanceGroup._get_resource_definition(self)

    def _try_rolling_update(self, prop_diff):
        return instgrp.InstanceGroup._try_rolling_update(self, prop_diff)

    def child_template_files(self, child_env):
        return instgrp.InstanceGroup.child_template_files(self, child_env)

    def get_attribute(self, key, *path):
        return instgrp.InstanceGroup.get_attribute(self, key, *path)

    def _nested_output_defns(self, resource_names, get_attr_fn, get_res_fn):
        return instgrp.InstanceGroup._nested_output_defns(
            self, resource_names, get_attr_fn, get_res_fn)

    def _create_template(self, num_instances, num_replace=0,
                         template_version=('HeatTemplateFormatVersion',
                                           '2012-12-12')):
        return super(AutoScalingGroup,
