...
class ServerNetworkMixin(object):
    ...
    def _validate_network(self, network):
        ...
        if port is None and self.is_using_neutron():
    ...
    def _create_internal_port(self, net_data, net_number):
        ...
        # in resource's data.
        self._port_data_add_item(port['id'])
    ...
    def _delete_internal_port(self, port_id):
        ...
        self._port_data_remove_item({'id': port_id})

    def _delete_internal_ports(self):
        for port_data in self._port_data_get_list():
            self._delete_internal_port(port_data['id'])

        self.data_delete('internal_ports')

    def _port_data_add_item(self, item):
        data = self._port_data_get_list()
        data.append({'id': item})
        self.data_set('internal_ports', jsonutils.dumps(data))

    def _port_data_remove_item(self, item):
        data = self._port_data_get_list()
        if item in data:
            data.remove(item)
        self.data_set('internal_ports', jsonutils.dumps(data))

    def _port_data_get_list(self):
